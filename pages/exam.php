<script type="text/javascript" >
   ...
   function preventBack(){window.history.forward();}
    ...
    function sendTimeToServer() {
        var examEndTime = new Date().getTime(); // Get the end time when the user navigates away
        var timeSpent = Math.floor((examEndTime - examStartTime) / 1000); // Calculate time spent in seconds

        // Ajax call to send time spent to server
        $.ajax({
            type: "POST",
            url: "save_time.php", // Replace with your server-side script to save time to the database
            data: {
                timeSpent: timeSpent
            },
            success: function(response) {
                console.log('Time sent to server: ' + timeSpent + ' seconds');
            }
        });
    }

    // Call the function when the user navigates away from the page
